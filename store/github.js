...
export const actions = {
  async fetchIssues (ctx) {
    ...
    const a11y_data = await axios.get(a11y_url).then(response => {
    ...
    const read_data = await axios.get(read_url).then(response => {
      return response.data;
    });
    ...
    const vue_data = await axios.get(vue_url).then(response => {
      return response.data;
    });
