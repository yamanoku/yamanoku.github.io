<template>
  <main>
    ...
    <article>
      ...
      <p>ヒューマンリーダブル（人が理解できる）とマシンリーダブル（機械が理解できる）な情報設計を実現するためにセマンティクスなマークアップ実装しています。</p>
      <p>支援技術を使用する人には不要となる要素について、WAI-ARIA（Web Accessibility Initiative - Accessible Rich Internet Applications）を使用した実装も行っています。</p>
      ...
      <highlight-code lang="vue">{{slide}}</highlight-code>
      <p>取得するAPIの整形は必要ですが、１つのコンポーネントで管理することで、ソースコードの簡略化・複数での使い回しや情報設計を効率化することができます。</p>
      <p>その他、コンポーネント指向開発でPropsが深くなってしまわないようにstoreから各APIを呼び出すように設定しています。</p>
      <highlight-code lang="javascript">{{api}}</highlight-code>
      <h4 id="i18n">
      ...
      <p>Nuxt.jsで開発しているのでnuxt-18nというツールを入れています。日本語と英語のほか、その他の言語対応もしていこうと思っています。</p>
      <ul role="list">
      ...
      <p>日本語UDフォント<a href="https://minmoji.ucda.jp/pages/webfont/" target="_blank" rel="noopener">みんなの文字&reg;WEBフォント</a>を使用しています。</p>
      <h4 id="font-size">
        <a class="anchor" aria-hidden="true" href="#font-size">
          <anchor-icon />
        </a>
      ...
      <p>使用しているカラーはいずれも、背景とのコントラスト比が最低4.5:1となるように設定されていますが、純粋な白黒ではなく、濃淡を抑えてバランスをとったカラーとなっています。CSSカスタム・プロパティ（CSS変数）を使用して指定しています。</p>
      <dl>
